...
import com.hurteng.stormplane.object.MoveTargetObj;
...
public class MainView extends BaseView {
    ...
    private Bitmap miku_fang; // 开始/暂停游戏的按钮图片
    ...
    public MainView(Context context, GameSoundPool sounds) {
        ...
        //button=new Button(R.drawable.button,screen_width,screen_height,resources);
        //public Button(int res,String text, Resources resources, Paint paint) {
        paint = new Paint();
        //AttributeSet attributeSet=new AttributeSet();
        //button=new Button(mainActivity);
        //button.setText("budada");
        //    public Button(int res,String text,float left,float top, Resources resources,Paint paint) {

        //button=new Button(R.drawable.button,"butt",resources,paint);
        button = new Button(R.drawable.button, "butt", 100, 100, resources, paint);
        //button.draw();

    }

    float getScreenCenterX(Bitmap bitmap) {
        return (float) (screen_width / 2 - bitmap.getWidth() * 1.0 / 2);
        //button_y = screen_height / 2 + button.getHeight();
    }

    float getScreenCenterY(Bitmap bitmap) {
        //return (float) ( screen_width / 2 - bitmap.getWidth()*1.0 / 2);
        return (float) (screen_height / 2 + bitmap.getHeight());
    }
    ...
    public void surfaceCreated(SurfaceHolder arg0) {
        ...
        initBitmap(); // 初始化图片资源
        //这里有的
        Log.d("miku_fang", "surfaceCreated: " + miku_fang);
        ...
        button.setScreenWidth(screen_width);
        //放在 屏幕的中间 是 左右的中间
        button.setScreenHeight(screen_height);
    }
    ...
    //boolean isTouched(int x,int y,int width,int height,int touchX,int touchY){
    boolean isTouched(int touchX, int touchY, int objX, int objY, int width, int height) {
        return touchX > objX && touchX < objX + width && touchY > objY && touchY < objY + height;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            float y = event.getY();
            // 在buttone 里面 开始游戏？
            Log.d("setTargetX ", "onTouchEvent: x " + x + " y " + y);
            itemMiku.setTargetX(x);
            itemMiku.setTargetY(y);
            itemMiku.setMoveDone(false);
    ...
    public void initBitmap() {
        ...
        playButton = BitmapFactory.decodeResource(getResources(),
                R.drawable.play);
        //这是 直接赋值的 后来覆盖 所以 这里有了之后才行
        miku_fang = BitmapFactory.decodeResource(getResources(),
                R.drawable.miku_fang);
        //itemMiku = new  MoveTargetObj(100,100,play_bt_w,play_bt_h,miku_fang);
        itemMiku = new MoveTargetObj(100, 100, 150, 50, miku_fang);
        itemMiku.setAlive(true);
    ...
    public void drawSelf() {
        try {
            ...
            //Item itemMiku = new Item(miku_fang,left,top,play_bt_w,play_bt_h);
            //GameObject itemMiku = new  GameObject(miku_fang,left,top,play_bt_w,play_bt_h);
            //GameObject itemMiku = new  MoveTargetObj(miku_fang,left,top,play_bt_w,play_bt_h);
            //GameObject itemMiku = new  MoveTargetObj(left,top,play_bt_w,play_bt_h,miku_fang);
            //itemMiku.drawSelf(canvas);
            //itemMiku.getObject_height()
            ...
            itemMiku.drawSelf(canvas);
            //itemMiku.setAlive(true);
            //if(	itemMiku.isAtTarget()){
            //	itemMiku.isMoveDone()
            //}
            if (itemMiku.isMoveDone()) {
                Log.d("TAG", "drawSelf:isMoveDone ");
                button.draw(canvas, paint);
            }
            //itemMiku.draw();
            ////在哪里点了一下
            //itemMiku.setLeft();
            //itemMiku.draw(canvas,paint);
            //x --- tx
...
////			canvas.clipRect(10, 10, 10 + play_bt_w, 10 + play_bt_h);
//
//			canvas.clipRect(left, top, 10 + play_bt_w, 10 + play_bt_h);
//
////			放在下面之后 把他覆盖了
////			做游戏的时候 坐标怎么知道的 怎么量出来
//			if (isPlay) {
//				canvas.drawBitmap(miku_fang, left, top, paint);
//			} else {
//				canvas.drawBitmap(miku_fang, left, top - play_bt_h, paint);
//			}
//			canvas.restore();
    ...
    //MoveTargetObj itemMiku = new  MoveTargetObj(100,100,play_bt_w,play_bt_h,miku_fang);
    MoveTargetObj itemMiku;
    //Item itemMiku = new Item(miku_fang,100,100,play_bt_w,play_bt_h);
